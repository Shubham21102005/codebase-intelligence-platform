...
async def ask_codebase(payload: AskRequest):
    ...
    hits = qdrant.search(collection_name=collection, query_vector=query_emb, limit=15)

    context_chunks = "\n\n".join([f"[File: {h.payload['file_path']}]\n{h.payload['content']}" for h in hits])

    # 2. Graph expansion
    files = list({h.payload['file_path'] for h in hits})
...
"""
    ...
    return {"answer": response.text, "sources": [h.payload['file_path'] for h in hits[:5]]}